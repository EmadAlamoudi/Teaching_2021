...
Using the boolean operators in R, validate the following expression: Suppose we have a variable `x`, equal to 12. Now check if this variable is divisible by 2 and at the same time `x` multiplied by 2 is smaller than `x` squared. In addition to the previous statement, be sure that `x` is not equal to 12 or `x` plus 10 is equal to 22. (Hint: the answer to that should be *true*)

```{r}
x <- 12
(x %% 2 == 0) && (2*x < x^2) && ((x != 12) || (x + 10 == 22))
```
...
sum_gauss <- function(n){
  return(n * (n + 1) / 2)
}

check <- function(){
  for(n in 1:10000){
    if(sum_naive(n) != sum_gauss(n)){
      return(FALSE)
      # As soon as the formulas do not coincide for a single value of n, the test is failed
    }
  }
  return(TRUE)
}
...
```{r}
is.divisible <- function(n, m){
  return(n %% m == 0)
}

is.prime <- function(n){
  if(n <= 1) return(FALSE) # 0 and 1 are not prime
  else if(n == 2) return(TRUE) # 2 is prime but not covered by the general case
  else{
    for(m in 2:(n-1)){ # NB 2:n-1 is something different: parentheses are needed here!
      if(is.divisible(n, m)){
        return(FALSE) # As soon as n has one divisors, we know it is not prime
      }
    }
    return(TRUE)
  }
...
# Print the first 10 prime numbers to be sure the function works
n <- 0
count <- 0
while(count < 10){
  if(is.prime(n)){
