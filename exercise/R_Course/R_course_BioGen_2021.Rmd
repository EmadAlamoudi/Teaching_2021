...
We can easily create a histogram to visualize the empirical distribution with `hist()` (by default, the bin widths are automatically estimated).
```{r}
hist(mtcars$mpg)
```

If we want a smooth approximation using Kernel Density Estimation (KDE),
the function `density()` can be used instead.
```{r}
hist(mtcars$mpg, freq=FALSE) # The histogram must represent densities to make it comparable to the KDE
lines(density(mtcars$mpg, bw="SJ")) # bw -> bandwidth estimation method
```

The empirical cumulative distribution function can be computed with `ecdf()` instead.
```{r}
plot(ecdf(mtcars$mpg), do.points=FALSE, verticals=TRUE)
```

As a final example, a quantile-quantile (Q-Q) plot can be used to compare the empirical distribution with a standard distribution. In the case a normal distribution is used for the comparison, the convenient shorthand `qqnorm()` can be used.
```{r}
qqnorm(mtcars$mpg)
qqline(mtcars$mpg) # Show "diagonal" for making comparison clearer
```
